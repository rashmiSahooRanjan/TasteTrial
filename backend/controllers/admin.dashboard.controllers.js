        ...
        const orderData = await Order.aggregate([
            { $match: { createdAt: { $gte: startDate } } },
            {
                $group: {
                    ...
                    cancelled: {
                        $sum: {
                            $cond: [{ $eq: ["$shopOrders.status", "cancelled"] }, 1, 0]
                        }
                    }
                }
            },
        ...
        const userGrowth = await User.aggregate([
            { $match: { createdAt: { $gte: startDate } } },
            {
                $group: {
                    ...
                    deliveryBoys: {
                        $sum: { $cond: [{ $eq: ["$role", "deliveryBoy"] }, 1, 0] }
                    }
                }
            },
            { $sort: { _id: 1 } }
...
export const getTopSellingFoods = async (req, res) => {
    try {
        ...
        const populated = await Item.populate(topFoods, {
            path: "_id",
            select: "name image price"
        });

        return res.status(200).json(populated.map(f => ({
            ...f,
            item: f._id,
            _id: f._id?._id || f._id
        })));
...
export const getTopRestaurants = async (req, res) => {
    try {
        ...
        const populated = await Shop.populate(topRestaurants, {
            path: "_id",
            select: "name image"
        });
...
export const getTopCustomers = async (req, res) => {
    try {
        ...
        const populated = await User.populate(topCustomers, {
            path: "_id",
...
export const getBestDeliveryBoys = async (req, res) => {
    try {
        ...
        const populated = await User.populate(bestBoys, {
            path: "_id",
            select: "fullName email mobile"
        });
